...
class TestNeoPoolEntityMigration:
    """Tests for NeoPoolEntity migration path."""

    def test_entity_with_entity_id_mapping(self, mock_config_entry: MagicMock) -> None:
        """Test entity uses old entity_id when migrating."""
        # Set up entity_id_mapping in runtime_data
        mock_config_entry.runtime_data.entity_id_mapping = {
            "water_temperature": "old_pool_water_temp"
        }

        entity = NeoPoolEntity(mock_config_entry, "water_temperature", "sensor")

        # Should use old entity_id from mapping
        assert entity.entity_id == "sensor.old_pool_water_temp"

    def test_entity_without_entity_id_mapping(self, mock_config_entry: MagicMock) -> None:
        """Test entity without mapping doesn't set entity_id."""
        # No entity_id_mapping
        mock_config_entry.runtime_data.entity_id_mapping = {}

        entity = NeoPoolEntity(mock_config_entry, "water_temperature", "sensor")

        # Should not have entity_id set (will be auto-generated by HA)
        assert not hasattr(entity, "entity_id") or entity.entity_id is None

    def test_entity_with_mapping_but_key_not_found(self, mock_config_entry: MagicMock) -> None:
        """Test entity when key is not in mapping."""
        mock_config_entry.runtime_data.entity_id_mapping = {"other_entity": "old_other_entity"}

        entity = NeoPoolEntity(mock_config_entry, "water_temperature", "sensor")

        # Should not have entity_id set
        assert not hasattr(entity, "entity_id") or entity.entity_id is None

    def test_entity_without_platform_domain(self, mock_config_entry: MagicMock) -> None:
        """Test entity without platform_domain doesn't use mapping."""
        mock_config_entry.runtime_data.entity_id_mapping = {
            "water_temperature": "old_pool_water_temp"
        }

        # No platform_domain provided
        entity = NeoPoolEntity(mock_config_entry, "water_temperature")

        # Should not have entity_id set (platform_domain is empty)
        assert not hasattr(entity, "entity_id") or entity.entity_id is None


class TestNeoPoolMQTTEntityMigration:
    """Tests for NeoPoolMQTTEntity migration path."""

    def test_mqtt_entity_with_entity_id_mapping(self, mock_config_entry: MagicMock) -> None:
        """Test MQTT entity uses old entity_id when migrating."""
        mock_config_entry.runtime_data.entity_id_mapping = {"ph_data": "old_pool_ph"}

        entity = NeoPoolMQTTEntity(mock_config_entry, "ph_data", "sensor")

        assert entity.entity_id == "sensor.old_pool_ph"
